...
**CRITICAL — personas MUST be passed via `--append-system-prompt-file`, NEVER inline via `--append-system-prompt <string>`.** On Windows the CLI is a `.cmd` file, so `spawn(CLAUDE_BIN, args, { shell: true })` is required. That routes through `cmd.exe`, whose argv parser is hostile to embedded double quotes. The personas in `AGENT_PERSONAS` contain literal `"` chars (`"y-yes Master"`, `"as you command, Master"`, etc.). When Node joins argv into a CMD command line, those embedded quotes close the `--append-system-prompt` argument prematurely, so CMD re-tokenizes the tail — and any flags that follow (`--dangerously-skip-permissions`, `--resume <uuid>`, `--allowed-tools`) get eaten as mangled persona fragments instead of reaching `claude.exe`. Symptom is a two-bug cluster hitting the **second+** command in a session (first works because no `--resume` yet): `[Permission mode: Normal]` even when Bypass is forced server-side, AND the agent reports "start of a new conversation" because the resume silently no-op'd. The fix writes the persona to a file in the agent workspace (`.persona-prompt`) and passes `--append-system-prompt-file <path>` — moving the only problematic arg off the command line entirely. All other args (uuids, model names, short flag values) are shell-safe. Do NOT "optimize" this back to inline, and if you ever add another long-string or quote-bearing flag, use the `-file` variant if one exists.

**CRITICAL — CMD arg quoting:** With `shell: true` on Windows, Node does NOT auto-quote args — it builds `cmd.exe /d /s /c "claude.cmd arg1 arg2 ..."` by plain-joining with spaces. Any arg containing whitespace will be split by `cmd.exe` at the space. The install path `D:\wolf vibecode\wolfs-basement\...` has a space, so the persona-file arg (e.g. `D:\wolf vibecode\wolfs-basement\workspaces\agent-1\.persona-prompt`) arrives at the CLI as just `D:\wolf` and fails with `Error: Append system prompt file not found: D:\wolf`. Fix: map every arg to `"${a}"` when it contains whitespace before passing to `spawn` — see the `cmdSafeArgs` mapper in `sendCommand`. If you ever add a new flag whose value could contain a space (file paths, free-form strings), the mapper already covers it. WSL branch is unaffected — it single-quotes every arg via `bash -c`.
